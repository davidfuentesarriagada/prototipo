...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO unidad_de_muestras3 (unidades_seleccionadas_adm, unidades_seleccionadas_auditoria, unidades_no_seleccionadas) 
            VALUES ('$unidadesSeleccionadasAdm', '$unidadesSeleccionadasAuditoria', '$unidadesNoSeleccionadas')";

    if ($conn->query($sql) === TRUE) {
        echo '<script>alert("Muestra guardada correctamente."); window.location.href = "http://localhost/prototipo/html/dashboard/table/BODEGA-29-RT-REPARACIONES.php";</script>';
    } else {
