  ...
  <body class="  ">
    ...
    <aside class="sidebar sidebar-default sidebar-white sidebar-base navs-rounded-all ">
      ...
      <div class="conatiner-fluid content-inner mt-n5 py-0">
        <?php
        // Establecer la conexión a la base de datos y realizar la consulta SQL
        ...
        $query = "SELECT id, nombres, apellidos, email, numero_telefonico, rol, fecha_nacimiento, direccion, edad FROM usuarios";
                ...
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <div class="header-title">
                                <h4 class="card-title"> Información de Usuarios Registrados </h4>
                            </div>
                        </div>
                        <div class="card-body">
                                ...
                                <table id="datatable" class="table table-striped" data-toggle="data-table">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo '<td>' . htmlspecialchars($row['numero_telefonico']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row['rol']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row['fecha_nacimiento']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row['direccion']) . '</td>';
                                            echo '<td>' . htmlspecialchars($row['edad']) . '</td>';
                                            echo '<td>
                                                    <button type="button" class="btn btn-primary btn-sm editUserBtn" data-toggle="modal" data-target="#modificarUsuarioModal" data-id="' . htmlspecialchars($row['id']) . '">Modificar</button>
                                                    <a href="eliminarUsuario.php?id=' . htmlspecialchars($row['id']) . '" class="btn btn-danger btn-sm">Eliminar</a>
                                                </td>';
                                            echo '</tr>';
                                        }
                                        ?>
        ...
        $query_modal = "SELECT id, nombres, apellidos, email, numero_telefonico, rol, fecha_nacimiento, direccion, edad FROM usuarios";
        ...
        <script>
            $(document).ready(function() {
                $('.editUserBtn').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#modificarUsuarioModal').find('#idUsuario').val(userData.id);
                            $('#modificarUsuarioModal').find('#nombres').val(userData.nombres);
                            $('#modificarUsuarioModal').find('#apellidos').val(userData.apellidos);
                            $('#modificarUsuarioModal').find('#email').val(userData.email);
                            $('#modificarUsuarioModal').find('#numero_telefonico').val(userData.numero_telefonico);
                            $('#modificarUsuarioModal').find('#contraseña').val(userData.contraseña); // No mostrar la contraseña
                            $('#modificarUsuarioModal').find('#rol').val(userData.rol);
                            $('#modificarUsuarioModal').find('#fecha_nacimiento').val(userData.fecha_nacimiento);
                            $('#modificarUsuarioModal').find('#direccion').val(userData.direccion);
                            $('#modificarUsuarioModal').find('#edad').val(userData.edad);
                ...
                $('#guardarCambios').on('click', function() {
                    var formData = $('#editarUsuarioForm').serialize(); // Serializa los datos del formulario

                    $.ajax({
                        url: 'updateUserData.php', // URL del script PHP para actualizar los datos
                        type: 'post',
                        data: formData,
                        success: function(response) {
                            // Manejar la respuesta del servidor
                            $('#modificarUsuarioModal').modal('hide');
                            // Actualizar la tabla o recargar la página para reflejar los cambios
                            location.reload();
                        },
                        error: function() {
        ...
        $username = "root";
        $password = "";
        $dbname = "prototipo";; // Asegúrate de tener un archivo config.php con los detalles de la base de datos
